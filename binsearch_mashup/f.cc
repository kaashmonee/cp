...
void solve() {
    ...
    long double det = powl(d, 2) - 4 * d;
    if (det < 0) {
        cout<<"N\n";
        return;
    }
    long double bpos = (d + sqrtl(det))/2;
    long double bneg = (d - sqrtl(det))/2;

    long double apos = d / bpos;
    long double aneg = d / bneg;

    if (apos < d && bpos < d) {
        cout<<"Y "<<fixed<<apos<<" "<<fixed<<bpos<<"\n";
        // printf("Y %lf %lf\n",(double) apos, (double) bpos);
        return;
    } 

    if (aneg < d && bneg < d) {
        cout<<"Y "<<fixed<<aneg<<" "<<fixed<<bneg<<"\n";
